...
@when(u'se envia un POST')
def step_impl(context):

    url_board = context.base_url+"/boards"
    query_board = context.query_params + ({"name":context.nameBoard},{"prefs_background":"lime"},{"defaultLists":"false"})
...
@then(u'El status codigo debe ser {numero:d}')
def step_impl(context,numero):
    expect(context.response.status_code).to_equal(numero)

# -----------------------------------------------------------------------
@when(u'se envia un metodo GET con el siguiente idChecklist')
def step_impl(context):
    for row in context.table:
      id=  row["idChecklist"]
    query = context.query_params
    url = context.base_url+"/checklists/"+id
    context.checklist = (request_util.get_request(url,query)).status_code


@then(u'el status code debe ser {numero:d}')
def step_impl(context,numero):
    expect(int(context.checklist)).to_equal(numero)
...
def step_impl(context):
    ...
    query_checkitems = context.query_params + ({"name": context.nameitem}, {"pos": context.positem},{"checked":context.checkeditem})
